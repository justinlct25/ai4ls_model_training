...
df, df_texture, df_bulk, df_chem = pd.read_csv("../../csv_processing/LUCAS-SOIL-2018(managed-l)(bulk-density)(erosion)(out-standard)(textural-info).csv")

# Replace values below LOD in specified columns
chem_attribute_columns = ['pH_H2O', 'EC', 'OC', 'CaCO3', 'P', 'N', 'K']
df_chem.fillna(0, inplace=True) # Replace NaN values with 0 in all columns
for column in chem_attribute_columns:
    df_chem[column] = df_chem[column].replace(['< LOD', '<  LOD', '<0.0'], 0).astype(float)


texture_attribute_columns = ['Coarse', 'Clay', 'Sand', 'Silt']
df_texture.dropna(subset=texture_attribute_columns, inplace=True)


bulk_density_column = ['BD 0-20']
df_bulk.dropna(subset=bulk_density_column, inplace=True)



# Extract features (X) and target variables (y)
features = df[["LU1_Desc", "LC0_Desc"]]
targets = df[phy_attribute_columns + chem_attribute_columns]
